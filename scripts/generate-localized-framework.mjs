...
const outputRoot = process.env.LOCALIZED_COURSE_OUTPUT ?? join(root, "src", "generated", "framework");

const locales = ["zh-CN", "en"];
const chapters = readdirSync(join(frameworkRoot, "chapters"))
...
const chapterMeta = new Map(chapters.map((file) => {
  const markdown = readFileSync(join(frameworkRoot, "chapters", file), "utf8");
...
function localizeLinks(text, locale) {
  return text.replace(/\[([^\]]+)\]\((framework-(\d{2})\.html)\)/g, (_all, _label, href, num) => {
...
function navigationLine(line, locale, currentNum) {
  if (!line.includes("framework-")) return null;
  const links = [...line.matchAll(/\[([^\]]+)\]\(framework-(\d{2})\.html\)/g)];
  if (!links.length) return null;
  return links.map((match) => {
    const num = match[2];
    const href = locale === "en" ? `en-framework-${num}.html` : `framework-${num}.html`;
    const title = chapterMeta.get(num)?.title[locale] ?? match[1];
    const label = Number(num) < Number(currentNum)
      ? (locale === "en" ? "Previous Chapter" : "上一章")
      : (locale === "en" ? "Next Chapter" : "下一章");
...
for (const locale of locales) {
  ...
  const glossary = readFileSync(join(frameworkRoot, "resources", "glossary-bilingual.md"), "utf8");
