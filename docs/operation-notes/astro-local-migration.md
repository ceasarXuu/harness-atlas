...
- Set `build.format: "file"` so routes build to flat HTML files such as `framework-01.html`, `products.html`, and other existing URL shapes.
...
- Keep shared route labels, homepage blocks, framework nodes, industry updates, and section cards in `src/data/site.mjs` so navigation and page structure are edited in one place.
- Load `assets/css/style.css` on every page and load `assets/css/framework.css` only on framework pages.
- Keep glossary content under the framework shell, currently `framework-glossary.html`; do not create or link a standalone top-level `glossary.html` route.
...
For the framework-site homepage, keep the structure vertical: header, hero, then industry updates. Size `.home-hero` so the first industry updates section starts inside the first viewport on desktop. On mobile, the runtime trace card can be hidden in the first hero because otherwise it pushes the industry updates below the first viewport.

## Framework Page Sidebar

The framework pages should use a `.framework-shell` two-column section without a top hero. Keep `.framework-sidebar` visible on every framework page and render framework nodes directly in the sidebar. The main sequence starts at `framework-01.html`; do not generate `course.html` as legacy route 00. Put the glossary under an `其他` group.
